...
class CDNet(nn.Module):
    def __init__(self, encoder, class_en, domain_en, classifier, domain_clf):
        super(CDNet, self).__init__()
        self.encoder = encoder
        self.class_en = class_en
        self.domain_en = domain_en
        self.classifier = classifier
        self.domain_clf = domain_clf

    def forward(self, x):
        features = self.encoder(x)
        class_fea = self.class_en(features)
        classes = self.classifier(class_fea)
        domain_fea = self.domain_en(features)
        domains = self.domain_clf(domain_fea)

        return classes, domains, class_fea, domain_fea
